...
запустити інші потоки, доки вони чекають виконання. Вони очікують на виконання в точці виклику методу 
condition.wait().
...
Якщо ж master повинен дозволити роботу лише одному з worker, можна викликати метод condition.notify(), тоді тільки 
один з тих, хто очікує дозволу worker продовжить роботу. Другий чекатиме, доки не буде виконано наступне 
condition.notify.
...
можуть встановлювати або скидати інші потоки. Для цього використовують метод set, щоб встановити прапор та метод 
clear для скидання. Методу wait класу Event зупиняє роботу потоку до того часу, доки інший потік не встановить 
прапор методом set. Є можливість перевірити, чи встановлено прапор методом is_set.
...
перезапуском та зупинкою роботи потоків через клас Event. Наприклад, у наступному прикладі ми перериваємо 
виконання потоку, який працює в нескінченному циклі та інакше просто ніколи не завершиться."""
...
кілька потоків продовжили роботу лише після того, як задане число потоків добереться у виконанні коду до цього 
"бар'єру".

Використовується, коли вам потрібно, щоб робота застосунку продовжилася лише після того, як усі потоки зроблять 
якусь частину своєї роботи та дійдуть до деякої точки, з якою можна знову продовжувати.
...
Потік може дістатися бар'єру і чекати його за допомогою функції wait(). Це блокуючий виклик, який повернеться, 
коли решта потоків (попередньо налаштована кількість barrier = Barrier(5)) дістануться бар'єру.
...
цього абстрактного базового класу: ProcessPoolExecutor — для виконання коду окремих процесів (з ним ми 
...
"""
У цьому прикладі ми створюємо ThreadPoolExecutor та задаємо, що робота буде виконуватися не більше ніж 2 потоками. 
Пул можна створити в менеджері контексту як у прикладі, щоб бути впевненим, що всі ресурси будуть коректно 
повернуті до системи після завершення. Але це не обов'язково, можна створити пул і потім викликати у нього метод 
shutdown, щоб завершити всі потоки та повернути ресурси системі.

Далі ми передаємо в пул функцію, яку потрібно виконати у кілька потоків, та набір аргументів цієї функції — кожен 
для виконання в окремому потоці. В executor є метод map, він використовується, коли потрібно паралельно виконати 
функцію з різними вхідними аргументами в окремих потоках.
...
ThreadPoolExecutor-0_0 та ThreadPoolExecutor-0_1. Результат повертається лише після того, як усі вхідні дані 
оброблені.
"""